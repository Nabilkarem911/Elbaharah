<template>
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      ...
      <PageHeader title="تقرير الموازنات" subtitle="حركة كل ماكينة" />
    </div>
    ...
    <div v-if="data" class="space-y-6">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="card p-4">
          ...
          <p class="text-xl font-bold text-success tabular-nums">{{ fmt(data.summary.total_after_fee) }}</p>
        </div>
        <div class="card p-4">
          <p class="text-xs text-gray-500">إجمالي الرسوم</p>
          <p class="text-xl font-bold text-danger tabular-nums">{{ fmt(data.summary.total_fees) }}</p>
        </div>
        <div class="card p-4">
          <p class="text-xs text-gray-500">عدد الحركات</p>
          <p class="text-xl font-bold tabular-nums">{{ data.summary.transactions_count }}</p>
        </div>
      </div>
...
const columns = [
  ...
  { key: 'transactions_count', label: 'عدد الحركات', type: 'number', sortable: true },
  { key: 'total_amount', label: 'إجمالي المبلغ', type: 'currency', sortable: true },
  { key: 'total_fees', label: 'الرسوم', type: 'currency' },
  { key: 'total_after_fee', label: 'بعد الرسوم', type: 'currency', sortable: true },
  { key: 'by_card.mada', label: 'مدى', type: 'currency' },
  { key: 'by_card.visa', label: 'فيزا', type: 'currency' },
  { key: 'by_card.mastercard', label: 'ماستر', type: 'currency' },
];
...
const load = async () => {
  ...
  try {
    ...
    const { data: res } = await api.get('/reports/pos', { params });
    data.value = res;
  } catch (err) {
    console.error(err);
  } finally {
