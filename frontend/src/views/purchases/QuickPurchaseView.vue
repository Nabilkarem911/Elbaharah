<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold text-primary-500">مشتريات سريعة</h2>
        <p class="text-sm text-gray-500 mt-1">إدخال متعدد الصفوف — اكتب رقم المورد وكود السمك</p>
      </div>
      <div class="flex items-center gap-3">
        <button @click="loadData" class="btn-ghost"><RefreshCw class="w-4 h-4" /> تحديث</button>
        <button @click="saveBatch" class="btn-gold" :disabled="saving">
    ...
    <p class="text-xs text-gray-400 text-center">
      اكتب رقم المورد → اضغط Enter → اكتب كود السمك → Enter → الوزن → Enter → السعر → Enter (صف جديد تلقائياً)
    </p>
  </div>
</template>
...
import { Plus, Trash2, Save, Loader2, RefreshCw } from 'lucide-vue-next';
import api from '../../api';
...
const calcRow = (row) => {
  // total_price is entered directly, weight is separate
  // price_per_kilo = total_price / weight
};
...
const saveBatch = async () => {
  ...
  try {
    ...
    toast(`تم حفظ ${data.created} فاتورة بنجاح`);
    rows.value = [];
    addRow();
