...
const todayPayments = computed(() => {
  ...
  return recentPayments.value.filter(p => p.sale_date === today).reduce((s, p) => s + Number(p.amount), 0);
});
const recentPayments = ref([]);

const loadAccounts = async () => {
  try {
    const [acctRes, salesRes] = await Promise.all([
      api.get('/credit/accounts', { params: { limit: 500 } }),
      api.get('/credit/sales', { params: { limit: 500, is_paid: true } }),
    ]);
    accounts.value = (acctRes.data.data || acctRes.data).filter(a => Number(a.total_balance) > 0);
    recentPayments.value = salesRes.data.data || salesRes.data;
  } catch (err) {
...
const processPayment = async () => {
  ...
  try {
    ...
    await api.post(`/credit/accounts/${acct.id}/settle`, {
    ...
    toast(err.response?.data?.error || 'فشل تسجيل الدفعة', 'error');
  } finally {
